...
class PhotoSelfieView extends StatelessWidget {
  final CameraPermissionContext cameraContext;

  // Constructor accepting the context (either item or selfie)
  const PhotoSelfieView({super.key, required this.cameraContext});
  ...
  void _handleCameraPermission(BuildContext context) {
    CameraPermissionHelper().checkAndRequestPermission(
      ...
      onClose: () { _navigateToWearOutfit(context); }, // Wrap the function in a closure
    );
  }

  // Navigate to EditOutfit route Tentatively not to work on wear outfit
  void _navigateToWearOutfit(BuildContext context) {
    Navigator.pushNamed(context, AppRoutes.createOutfit);
  }
}
