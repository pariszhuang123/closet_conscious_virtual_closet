    ...
    WHERE
      ...
      AND (
        ...
        OR (
          ...
          -- Conditional colour variations filter: Only apply if colour is not black or white
          AND (
            sp.filter->'colour_variations' IS NULL
            OR (
              i.colour NOT IN ('black', 'white')
              AND i.colour_variations = ANY(ARRAY(SELECT jsonb_array_elements_text(sp.filter->'colour_variations')))
            )
          )

          -- Conditional subtype filters based on item type
          AND (
            (i.item_type = 'accessory' AND (sp.filter->'accessory_type' IS NULL OR a.accessory_type = ANY(ARRAY(SELECT jsonb_array_elements_text(sp.filter->'accessory_type')))))
            OR (i.item_type = 'clothing' AND (
                (sp.filter->'clothing_type' IS NULL OR c.clothing_type = ANY(ARRAY(SELECT jsonb_array_elements_text(sp.filter->'clothing_type'))))
                AND (sp.filter->'clothing_layer' IS NULL OR c.clothing_layer = ANY(ARRAY(SELECT jsonb_array_elements_text(sp.filter->'clothing_layer'))))
            ))
            OR (i.item_type = 'shoes' AND (sp.filter->'shoes_type' IS NULL OR s.shoes_type = ANY(ARRAY(SELECT jsonb_array_elements_text(sp.filter->'shoes_type')))))
          )
        )
      )
