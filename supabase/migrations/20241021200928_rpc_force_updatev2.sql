...
AS $$
DECLARE
  -- Variables to store major, minor, and patch components of each version
  ...
  -- Variables to store cleaned versions without non-numeric parts
  clean_version1 VARCHAR;
  clean_version2 VARCHAR;

BEGIN
  -- Remove any non-numeric suffix from version1 (like -dev, -beta)
  clean_version1 := regexp_replace(version1, '[^0-9.]+$', '', 'g');

  -- Remove any non-numeric suffix from version2 (like -dev, -beta)
  clean_version2 := regexp_replace(version2, '[^0-9.]+$', '', 'g');

  -- Split version1 into major, minor, patch with defaults
  SELECT COALESCE(NULLIF(split_part(clean_version1, '.', 1), ''), '0')::INT,
         COALESCE(NULLIF(split_part(clean_version1, '.', 2), ''), '0')::INT,
         COALESCE(NULLIF(split_part(clean_version1, '.', 3), ''), '0')::INT
  INTO v1_major, v1_minor, v1_patch;

  -- Split version2 into major, minor, patch with defaults
  SELECT COALESCE(NULLIF(split_part(clean_version2, '.', 1), ''), '0')::INT,
         COALESCE(NULLIF(split_part(clean_version2, '.', 2), ''), '0')::INT,
         COALESCE(NULLIF(split_part(clean_version2, '.', 3), ''), '0')::INT
