...
BEGIN
    ...
    INTO daily_upload_status, has_closet_uploaded
    ...
    SET
        outfits_created = outfits_created + 1,
        no_buy_highest_streak = CASE
                                    WHEN daily_upload_status IS FALSE
                                        AND has_closet_uploaded
                                        AND no_buy_streak = no_buy_highest_streak
                                    THEN no_buy_highest_streak + 1
                                    ELSE no_buy_highest_streak END,
        no_buy_streak = CASE WHEN daily_upload_status IS FALSE AND has_closet_uploaded THEN no_buy_streak + 1 ELSE no_buy_streak END,
        daily_upload = TRUE,
        updated_at = NOW()
...
DECLARE
    current_user_id UUID := auth.uid();
    unworn_initial_items_count INTEGER;
BEGIN
    -- First, count the items that are part of the initial upload and have not been worn before
    SELECT COUNT(*)
    INTO unworn_initial_items_count
    FROM public.items
    WHERE item_id = ANY(p_selected_items)
      AND closet_upload = TRUE
      AND worn_in_outfit = 0;

    -- Then, update worn_in_outfit and item_last_worn fields for selected items
    UPDATE public.items
    ...
    -- Finally, update initial_upload_items_worn_count for the user
    UPDATE public.user_high_freq_stats
    SET
        initial_upload_items_worn_count = initial_upload_items_worn_count + unworn_initial_items_count,
        updated_at = NOW()
    WHERE user_id = current_user_id;
END;
$$;
...
BEGIN
    ...
    PERFORM public.increment_outfit_created();
    PERFORM public.increment_items_usage(p_selected_items);
