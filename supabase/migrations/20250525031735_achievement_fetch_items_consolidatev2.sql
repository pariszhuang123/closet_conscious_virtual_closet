...
BEGIN
  ...
  -- 2) dynamic SQL with FORMAT() + a single USING for p_category
  RETURN QUERY EXECUTE FORMAT($sql$
    SELECT
      ...
      i.item_last_worn
    FROM public.items i
    LEFT JOIN public.items_accessory_basic a
      ON i.item_id = a.item_id
      AND i.item_type = 'accessory'
    LEFT JOIN public.items_clothing_basic c
      ON i.item_id = c.item_id
      AND i.item_type = 'clothing'
    LEFT JOIN public.items_shoes_basic s
      ON i.item_id = s.item_id
      AND i.item_type = 'shoes'
    JOIN public.shared_preferences sp
      ON i.current_owner_id = sp.user_id
    WHERE
      i.is_active = TRUE
      AND i.is_pending = FALSE

      -- ← back to a parameter here
      AND ($1 IS NULL OR i.item_type = $1)
      ...
      AND (
        sp.ignore_item_name
        OR (
          sp.item_name IS NOT NULL
          AND i.name ILIKE '%%' || sp.item_name || '%%'
        )
      )

      AND (
        sp.filter = '{}'::jsonb
        OR sp.filter IS NULL
        OR (
    ...
    ORDER BY
      i.%I %s,   -- dynamic sort column & direction
  ...
  USING p_category;  -- <— safely bind the category parameter
