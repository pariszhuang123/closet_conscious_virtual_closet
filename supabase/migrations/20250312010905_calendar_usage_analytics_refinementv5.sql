...
BEGIN
    ...
    ----------------------------------------------------------------------------
    -- Step 2: Use a CTE to filter outfits dynamically (no temp table)
    ----------------------------------------------------------------------------
    ...
    ----------------------------------------------------------------------------
    SELECT
        EXISTS (SELECT 1 FROM filtered_outfits WHERE created_day < f_focused_date LIMIT 1),
        EXISTS (SELECT 1 FROM filtered_outfits WHERE created_day > f_focused_date LIMIT 1)
    INTO has_previous_outfits, has_next_outfits;

    ----------------------------------------------------------------------------
    -- Step 4: Fetch daily outfits using JSON aggregation (Sorted by `updated_at DESC`)
    ----------------------------------------------------------------------------
    SELECT JSONB_AGG(
        JSONB_BUILD_OBJECT(
    ...
    FROM (
        ...
        ORDER BY updated_at DESC  -- ✅ Sorting applied correctly
    ) opd
    LEFT JOIN (
        SELECT
            oi.outfit_id,
        ...
        GROUP BY oi.outfit_id  -- ✅ Ensures proper item grouping
    ) di
