DROP FUNCTION IF EXISTS public.award_user_achievements();
DROP FUNCTION IF EXISTS public.consolidate_fetch_items_with_preferences(
  integer,  -- p_current_page
  text      -- p_category
);
...
END IF;
  ...
  IF EXISTS (
       SELECT 1
        ...
        WHERE uhfs.user_id = current_user_id
          AND uhfs.initial_upload_items_worn_percentage >= 100
     )
     AND NOT EXISTS (
       SELECT 1
  ...
  THEN
    ...
    INSERT INTO public.user_achievements (
      user_id, achievement_name, awarded_at
    )
    VALUES (
      current_user_id,
      'all_clothes_worn',
    ...
    -- 2) build the JSON result for the badge itself
    SELECT jsonb_build_object(
      'status',           'success',
      'achievement_name', a.achievement_name,
      'badge_url',        a.badge_url
    ...
    -- 3) grant the “arrange” feature if not already in premium_services
    ...
    IF NOT EXISTS (
         SELECT 1
          ...
          WHERE ps.user_id = current_user_id
            AND (ps.one_off_features ? feature_name)
       )
    THEN
      ...
      RETURN result || jsonb_build_object('feature', 'activated');
    ELSE
      RETURN result || jsonb_build_object('feature', 'already_exists');
    END IF;
  END IF;
  ...
  IF p_achievement_name IS NOT NULL AND NOT EXISTS (
    ...
    IF feature_name IS NULL THEN
      RETURN result || jsonb_build_object('feature', 'none');
    END IF;
