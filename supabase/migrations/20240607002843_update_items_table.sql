...
ALTER TABLE items
ADD COLUMN current_owner_id UUID REFERENCES users(user_id) ON DELETE CASCADE;
...
COMMENT ON COLUMN items.upload_owner_id IS 'Identifier for the user who originally uploaded the item. This field helps track who first added the item to the database.';

-- Commenting on the current_owner_id column
COMMENT ON COLUMN items.current_owner_id IS 'Identifier for the current owner of the item. This field is updated to reflect changes in ownership, such as through swaps or transfers.';
