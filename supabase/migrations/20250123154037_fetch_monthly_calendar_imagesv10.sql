...
BEGIN
    ...
    -- Step 8: Fetch calendar outfits, grouping by created_day
    WITH calendar_outfits AS (
        SELECT
            ...
            JSONB_AGG(
                JSONB_BUILD_OBJECT(
                    ...
                    'outfit_image_url', o.outfit_image_url,  -- Could be 'cc_none'
                    'items', CASE
        ...
        GROUP BY o.created_day
        ORDER BY o.created_day
    )
    SELECT JSONB_AGG(
        JSONB_BUILD_OBJECT(
            'date', c.created_day,
            'outfit_data', c.outfits_for_day
        )
    )
    INTO result
