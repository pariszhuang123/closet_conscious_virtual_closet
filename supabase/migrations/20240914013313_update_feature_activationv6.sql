CREATE OR REPLACE FUNCTION public.achievement_badge(p_achievement_name TEXT)
RETURNS JSON
SET search_path = ''
LANGUAGE plpgsql
AS $$
...
CREATE OR REPLACE FUNCTION public.achievement_milestone_reward(
...
BEGIN
    ...
    -- Step 2: Fetch badge URL from achievements table
    SELECT json_build_object(
        'status', 'success',
        'badge_url', COALESCE(badge_url, 'https://vrhytwexijijwhlicqfw.supabase.co/storage/v1/object/public/closet-conscious-assets/ClosetConscious/Small_CC_Logo.png')
    ) INTO result
    ...
    -- Step 3: Check if the feature already exists in users table
