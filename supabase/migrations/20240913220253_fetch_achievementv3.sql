...
begin
  ...
  if clothes_worn then
    achievement_name := 'all_clothes_worn';
  end if;

  -- Return JSON object with status and achievement name (or null if not achieved)
    ...
    'no_new_clothes_225',
      case
        when current_streak = 225 and not exists (
          select 1 from public.user_achievements where user_id = current_user_id and achievement_name = 'no_new_clothes_225'
        ) then true else null end,
    'no_new_clothes_405',
      case
        when current_streak = 405 and not exists (
          select 1 from public.user_achievements where user_id = current_user_id and achievement_name = 'no_new_clothes_405'
        ) then true else null end,
    'no_new_clothes_630',
      case
        when current_streak = 630 and not exists (
          select 1 from public.user_achievements where user_id = current_user_id and achievement_name = 'no_new_clothes_630'
        ) then true else null end,
    'no_new_clothes_900',
      case
        when current_streak = 900 and not exists (
          select 1 from public.user_achievements where user_id = current_user_id and achievement_name = 'no_new_clothes_900'
        ) then true else null end,
    'no_new_clothes_1215',
      case
        when current_streak = 1215 and not exists (
          select 1 from public.user_achievements where user_id = current_user_id and achievement_name = 'no_new_clothes_1215'
        ) then true else null end,
    'no_new_clothes_1575',
      case
        when current_streak = 1575 and not exists (
          select 1 from public.user_achievements where user_id = current_user_id and achievement_name = 'no_new_clothes_1575'
        ) then true else null end
